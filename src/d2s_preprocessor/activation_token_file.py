...
def get_vat_act_token_file_path():
    ...
    if not res.exists():
        raise FileNotFoundError(f"File Token for VAT not found: {res}")
    return res
...
def get_invoice_number_token_file_path():
    ...
    if not res.exists():
        raise FileNotFoundError(f"File Token for Invoice Number not found {res}")
    return res
...
def get_amounts_activation_token_path():
    ...
    if not res.exists():
        raise FileNotFoundError(f"File Token for Amounts not found: {res}")
    return res
